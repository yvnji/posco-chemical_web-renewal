...
window.addEventListener('scroll', function () {
  ...
  // IR
  irBox.classList.toggle(
    'on',
    sct >= document.querySelector('.ir').getBoundingClientRect().top + window.pageYOffset - 200
  );
});
...
const mainSlideStop = document.querySelector('.main-slide_stop');
const mainSlidePlay = document.querySelector('.main-slide_play');
mainSlideStop.addEventListener('click', function () {
  mainS.autoplay.stop();
  this.style.display = 'none';
  mainSlidePlay.style.display = 'block';
});

mainSlidePlay.addEventListener('click', function () {
  mainS.autoplay.start();
  this.style.display = 'none';
  mainSlideStop.style.display = 'block';
});
